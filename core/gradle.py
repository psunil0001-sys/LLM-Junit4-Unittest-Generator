...
def run_gradle(
    ...
    *,
    ...
    from UnitTest_gen.core.emulator import (
        device_available,
        emulator_session_if_needed,
        ensure_running,
        gradle_needs_device,
    )

    needs_device = gradle_needs_device(selected_tasks)
    with emulator_session_if_needed(selected_tasks):
        if needs_device:
            if config.auto_start_emulator:
                if not ensure_running():
                    text = (
                        f"{header}EXIT_CODE: 1\nBUILD FAILED\n"
                        "No adb device available and AOSP emulator failed to start "
                        "(required for connected androidTest / instrumentedCoverageReport).\n"
                    )
                    return GradleRunResult(full_output=text, agent_tail=text)
            elif not device_available():
                text = (
                    f"{header}EXIT_CODE: 1\nBUILD FAILED\n"
                    "No adb device available and TESTGEN_AUTO_START_EMULATOR is off "
                    "(required for connected androidTest / instrumentedCoverageReport).\n"
                )
                return GradleRunResult(full_output=text, agent_tail=text)
        # Owned emulator stays up until generator exit / atexit (not after this Gradle call).
