"""Semgrep-core scan helper for Kotlin static policy."""
...
def require_semgrep_executable(executable: str = "") -> str:
    candidates = [executable, get_config().semgrep_core_executable, shutil.which("semgrep-core") or ""]
...
def scan_text_with_semgrep(
    ...
    for config_path in rule_configs:
        ...
        try:
            payload = yaml.safe_load(file_cache.read_text(config_path)) or {}
        except (file_cache.FileCacheError, yaml.YAMLError) as exc:
    ...
    try:
        ...
        if source_path and os.path.isfile(source_path):
            on_disk = file_cache.read_text(source_path, default="")
            if on_disk == (source_text or ""):
...
def _semgrep_work_dir() -> str:
    path = get_config().semgrep_cache_dir or os.path.join(
