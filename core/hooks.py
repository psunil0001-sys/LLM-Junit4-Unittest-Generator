"""Pre/PostToolUse discovery hooks for Claude Code."""
...
_PLAN_MUTATION_DENY = "planner may only Write/Edit *.plan.md"
_CODER_MUTATION_DENY = (
    "Write/Edit only under src/test, src/androidTest, build.gradle.kts, build.gradle, "
    "or UnitTest_gen — not src/main or other paths."
)
_GRADLE_BUILD_FILENAMES = frozenset({"build.gradle.kts", "build.gradle"})
_READ_ONCE_DENY = (
    "Read-once: file unchanged since earlier Read. Use that tool_result; do not re-Read."
)
...
_DISCOVERY_CMD = re.compile(r"\b(find|bfs|grep|ugrep)\b")
_PIPELINE_DENY_CMD = re.compile(
    r"\b(\./gradlew|gradlew|gradle\b|rm\b|curl|wget|python3|pip\b|npm\b|ssh\b|scp\b)\b"
)
...
def parse_source_import_paths(prompt_text: str) -> dict[str, str]:
    """Map unique basenames from SOURCE IMPORTS absolute paths. Skip duplicate names."""
...
def decide_read_once(
    ...
    """Deny full-file re-Read when mtime unchanged; else record and return None."""
    ...
    stored = by_path.get(resolved)
    if stored is not None and stored == mtime_ns:
        return ReadHookDecision(
            permission="deny",
            file_path=resolved,
            reason=_READ_ONCE_DENY,
        )
...
def decide_read_path(file_path: str, by_basename: dict[str, str] | None = None) -> ReadHookDecision:
    ...
    if not requested:
        return ReadHookDecision(
            ...
            file_path="",
            reason=(
                "Read file_path is empty. Copy an absolute path from SOURCE IMPORTS "
                "or Bash find under OWNING MODULE DIR once."
            ),
        )
    ...
    if listed_ok:
        ...
        return ReadHookDecision(
            ...
            updated=True,
            reason=f"Rewrote Read to SOURCE IMPORTS path for {name}",
        )
...
def decide_plan_mutation_path(file_path: str) -> ReadHookDecision:
    """Planner may Write/Edit only ``*.plan.md`` (prefer paths under ``plans_dir()``)."""
    requested = (file_path or "").strip()
    if not requested:
        return ReadHookDecision(
            permission="deny",
...
def decide_edit_path(file_path: str, by_basename: dict[str, str] | None = None) -> ReadHookDecision:
    """Plan: ``*.plan.md`` only. Coder/fix: test roots + UnitTest_gen only."""
    _ = by_basename
    if _agent_phase_is_plan():
        decision = decide_plan_mutation_path(file_path)
        if decision.permission == "allow" and not Path(decision.file_path).is_file():
            return ReadHookDecision(
                permission="deny",
...
def _pretool_mutation_response(
    ...
    *,
    ...
    if not isinstance(tool_input, dict):
        tool_input = {}
    requested = str(tool_input.get("file_path") or tool_input.get("path") or "")
    if tool_name == "Write" and _agent_phase_is_plan():
        decision = decide_plan_mutation_path(requested)
    elif tool_name == "Write":
        decision = decide_coder_mutation_path(requested)
    else:
...
def claude_read_hook_settings_path() -> Path:
    return Path(__file__).resolve().parents[1] / ".claude" / "settings.json"

def claude_read_hook_script_path() -> Path:
    return Path(__file__).resolve().parents[1] / ".claude" / "hooks" / "remap_read_path.py"

def _pretool_hook_entry(script: Path) -> dict:
    return {
        "type": "command",
        "command": "python3",
...
def write_claude_read_hook_settings(dest: Path | None = None) -> Path:
    """Write Claude PreToolUse settings with an absolute hook script path."""
    script = claude_read_hook_script_path()
    if dest is None:
        handle, name = tempfile.mkstemp(prefix="testgen_claude_settings_", suffix=".json")
        os.close(handle)
        dest = Path(name)
    payload = {
        "hooks": {
            "PreToolUse": [
                {"matcher": "Read", "hooks": [_pretool_hook_entry(script)]},
                {"matcher": "Edit", "hooks": [_pretool_hook_entry(script)]},
                {"matcher": "Write", "hooks": [_pretool_hook_entry(script)]},
                {"matcher": "Bash", "hooks": [_pretool_hook_entry(script)]},
            ],
            "PostToolUse": [
                {"matcher": "Bash", "hooks": [_pretool_hook_entry(script)]},
                {"matcher": "Read", "hooks": [_pretool_hook_entry(script)]},
...
def classify_bash_command(command: str) -> str:
    """Return empty | pipeline_deny | not_discovery | discovery."""
    cmd = (command or "").strip()
    if not cmd:
        return "empty"
    if _PIPELINE_DENY_CMD.search(cmd):
        return "pipeline_deny"
    if not _DISCOVERY_CMD.search(cmd):
        return "not_discovery"
    normalized = cmd.replace("\\", "/")
    if re.search(r"\b(find|bfs)\b", cmd) and (
        "build/" in normalized or re.search(r"-path\s+['\"]?[^'\"]*build/", normalized)
    ):
        return "pipeline_deny"
    return "discovery"
...
def decide_bash_input(command: str) -> ReadHookDecision:
    """Allow Bash only for find/grep discovery; deny pipeline/destructive commands."""
    cmd = (command or "").strip()
    ...
    if kind == "empty":
        return ReadHookDecision(
            permission="deny",
            file_path="",
            reason="Bash command is empty. Use find or grep under OWNING MODULE DIR for discovery.",
        )
    if kind == "pipeline_deny":
        return ReadHookDecision(
            permission="deny",
            file_path="",
            reason=(
                "Bash denied: pipeline runs Gradle verify. "
                "Use find/grep discovery only — never gradlew, rm, python3, or build/ scans."
            ),
        )
    if kind == "not_discovery":
        return ReadHookDecision(
            permission="deny",
            file_path="",
            reason=(
                "Bash allowed only for find/grep discovery (embedded bfs/ugrep). "
                "Example: find /abs/module -name '*nav_graph.xml'"
            ),
        )
    if is_glob_grep_locked():
        module = owning_module_dir()
