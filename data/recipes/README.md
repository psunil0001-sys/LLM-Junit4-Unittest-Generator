...
`osmdroid_mapview`) never redefine attach — catalog pastes MUST stubs only. Up to **two**
companions may be selected (priority: CarUi → Navigation → Permissions → osmdroid).
...
`catalog_context._select_primary_recipe()` is authoritative.
