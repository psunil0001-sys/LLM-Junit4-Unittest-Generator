...
from UnitTest_gen.core.agent import run_agent as run_claude
...
def agent_reported_gradle_success(text: str) -> bool:
    """True when agent stdout/jsonl shows a later BUILD SUCCESSFUL than BUILD FAILED."""
    blob = text or ""
    ok = blob.rfind("BUILD SUCCESSFUL")
    if ok < 0:
        return False
    fail = blob.rfind("BUILD FAILED")
    return fail < ok
...
class _CoderRound:
    ...
    suspected_source_bug: bool = False
    suspected_evidence: str = ""
...
async def _run_coder_slice(
    *,
    ...
    compile_command = ""
    if test_layer == TestLayer.INSTRUMENTED:
        compile_invocation = resolve_instrumented_compile_invocation(
            project_root=project_root,
            test_file_path=test_path,
            cli_tasks=acceptance_tasks,
            gradle_output=baseline_output,
        )
        compile_command = format_gradle_command(compile_invocation)
        if skip_device_verify:
            fast_invocation = compile_invocation
            gradle_command = compile_command
        else:
    ...
    last_agent_stdout = getattr(gen_result, "stdout", "") or ""

    if not Path(test_path).is_file() or not file_cache.read_text(test_path, default="").strip():
        # Model often ends a long discovery turn with analysis text and no Write.
        # One short force-Write retry before treating the slice as empty_output.
        log_message(
            f"⚠️ Slice tag={slice_tag} TARGET missing after coder — forcing one Write retry.",
            category="warning",
        )
        force_user = (
            f"PIPELINE TARGET is still missing or empty.\n"
            f"Call Write NOW with file_path exactly:\n{test_path}\n"
            f"Content: a complete JUnit4 Kotlin class (package, imports, class name = file stem, "
            f"annotations/rules, @Test methods for THIS SLICE only). "
            f"Do not Read/Glob/Grep. Do not reply with analysis — only the Write tool call."
        )
        try:
            gen_result = run_claude(
                force_user,
                cwd=project_root,
                system_prompt=(
                    "You are the UnitTest_gen coder. The TARGET file was not created. "
                    "Your only job this turn is Write the PIPELINE TARGET path given in the user "
                    "message with a complete JUnit4 test class."
                ),
                title=f"GENERATION FORCE-WRITE (slice={slice_tag})",
                temperature=config.gen_temperature,
                presence_penalty=config.gen_presence_penalty,
                available_tools=CODER_AVAILABLE_TOOLS,
            ...
            last_agent_stdout = getattr(gen_result, "stdout", "") or last_agent_stdout
        except (AgentUnavailableError, OSError) as exc:
            return _SliceOutcome(False, False, [], unavailable=True, evidence=str(exc))
    ...
    for attempt in range(config.fix_max_attempts + 1):
        ...
        if agent_reported_gradle_success(last_agent_stdout):
            log_message(
                "⏭️ Skipping Python FAST_VERIFY — agent already reported BUILD SUCCESSFUL.",
                category="info",
            )
            gradle_ok = True
            full_gradle_output = last_agent_stdout
        else:
        ...
        if gradle_ok and not new_issues:
            break
        if not gradle_ok and classify_gradle_failure(full_gradle_output) == "contract":
            log_message(
                "🪲 Contract assertion failure — skipping further fix passes (do not weaken asserts).",
                category="warning",
            )
            break
        ...
        try:
            ...
            _log_edit_no_match(
                ...
                slice_tag=slice_tag)
            last_agent_stdout = getattr(fix_result, "stdout", "") or ""
    ...
    if new_issues or not gradle_ok:
        if not gradle_ok and classify_gradle_failure(full_gradle_output) == "contract":
            return _SliceOutcome(
                False, False, new_issues,
                evidence="Suspected SOURCE contract failure (assertion trap).",
                failure_stage="suspected_source_bug",
                gradle_output=full_gradle_output,
            )
        evidence = (
...
async def _run_coder_slices(
    *,
    ...
    for index, (tag, slice_plan) in enumerate(slices):
        ...
        if getattr(outcome, "failure_stage", "") == "suspected_source_bug":
            from UnitTest_gen.kotlin.coverage import persist_suspected_bug, merge_unreachable_entries
            from UnitTest_gen.kotlin.project import module_path_for_dir

            test_code = file_cache.read_text(test_path, default="")
            row = persist_suspected_bug(
                source_file_path=source_file_path,
                test_file_path=test_path,
                test_code=test_code,
                module=module_path_for_dir(project_root, module_dir),
                slice_tag=tag,
                lines=testable_lines(slice_plan),
                gradle_output=getattr(outcome, "gradle_output", "") or "",
            )
            line_vals = sorted(testable_lines(slice_plan))
            if line_vals:
                    ...
                    [
                        {
                            ...
                            "module": module_path_for_dir(project_root, module_dir),
                            "lines": line_vals,
                            "reason": "Assertion trap: JUnit contract failure; suspected production bug.",
                            "category": "suspected_production_bug",
                            "evidence": "assertion_trap",
                        }
                    ]
                )
            sidecar = str(row.get("sidecar") or "")
            suspected_source_bug = True
            suspected_evidence = (
                f"Suspected SOURCE contract failure. Sidecar: {sidecar}"
                if sidecar
                else "Suspected SOURCE contract failure (assertion trap)."
            )
            log_message(
                f"🪲 Slice tag={tag} assertion trap saved to {sidecar}; restoring last green TARGET.",
                category="warning",
            )
        else:
            log_message(
                f"↩️ Slice tag={tag} failed ({reason}); restored last green snapshot; continuing.",
                category="warning",
            )
        _write_test_snapshot(test_path, last_green_existed, last_green_contents)
    ...
    return _CoderRound(
        ...
        suspected_source_bug=suspected_source_bug, suspected_evidence=suspected_evidence)

import os

from UnitTest_gen.core.io import log_message
from UnitTest_gen.core.config import get_config, run_instrumented_tests
from UnitTest_gen.core.emulator import emulator_session, instrumented_device_planned
from UnitTest_gen.kotlin.project import (
    GradleInvocation,
    format_gradle_command,
    resolve_instrumented_gate_tasks,
...
async def run_instrumented_pass(
    *,
    ...
    """Generate androidTests when test_mode allows; pipeline may start AOSP for connected verify."""
    config = get_config()
    ...
    with emulator_session():
        return await _run_instrumented_pass_body(
            plan=plan,
            source_file_path=source_file_path,
            source_root=source_root,
            project_root=project_root,
...
async def _run_instrumented_pass_body(
    *,
    plan,
    source_file_path: str,
    source_root: str,
    ...
    skip_verify = not instrumented_device_planned()

    if skip_verify:
        log_message(
            "⚠️ Instrumented mode: no adb device and AOSP emulator cannot be started — "
            "will generate androidTest and skip connected verify.",
            category="warning",
        )
    else:
        log_message(
            "📱 Instrumented pass: connected androidTest + JaCoCo "
            "(AOSP emulator starts automatically if none is attached).",
            category="info",
        )
    ...
    from UnitTest_gen.kotlin.pipeline import refresh_last_green_snapshot
...
from UnitTest_gen.kotlin.codegen import read_test_file_state
from UnitTest_gen.kotlin.codegen import (
    baseline_failure_is_fixable,
    classify_gradle_failure,
    format_unrelated_baseline_error_message,
)
...
from UnitTest_gen.kotlin.project import (
    GradleInvocation,
    cli_acceptance_tasks,
    drop_device_acceptance_tasks,
    filter_device_acceptance_tasks,
    format_gradle_command,
)
...
async def _repair_baseline_gradle(
    *,
    project_root: str,
    source_file_path: str,
    source_code: str,
    ...
    """Run fixer on source-related baseline failures; re-run acceptance Gradle each pass.

    Returns ``(ok, gradle_result)`` where ``ok`` means acceptance tasks are green.
    """
    from UnitTest_gen.core.agent import AgentUnavailableError
    from UnitTest_gen.core.agent import run_agent as run_claude
    from UnitTest_gen.core.config import (
    ...
    from UnitTest_gen.kotlin.codegen import RepairTicket, build_repair_tickets, format_target_excerpt
    from UnitTest_gen.kotlin.layer import TestLayer
    from UnitTest_gen.kotlin.prompts import build_fix_prompt
    from UnitTest_gen.kotlin.project import GradleInvocation, format_gradle_command

    config = get_config()
    agent_timeout = max(config.agent_timeout_seconds, config.gradle_timeout_seconds)
    gradle_command = format_gradle_command(
        GradleInvocation(tasks=list(acceptance_tasks), gradle_args=[])
    )
    owning_module = find_owning_module_dir(project_root, source_file_path)

    # Prefer an existing TARGET the errors likely touch; else unit path for Write.
    primary_target = test_path
    if Path(instr_path).is_file() and (
        Path(instr_path).name in (baseline_output or "")
        or Path(instr_path).stem in (baseline_output or "")
    ):
        primary_target = instr_path
    elif not Path(test_path).is_file() and Path(instr_path).is_file():
        primary_target = instr_path

    test_layer = (
        TestLayer.INSTRUMENTED
        if "/androidTest/" in primary_target.replace("\\", "/")
        else TestLayer.UNIT
    )
    compile_command = ""
    if test_layer == TestLayer.INSTRUMENTED:
        from UnitTest_gen.kotlin.project import resolve_instrumented_compile_invocation

        compile_command = format_gradle_command(
            resolve_instrumented_compile_invocation(
                project_root=project_root,
                test_file_path=primary_target,
                cli_tasks=acceptance_tasks,
                gradle_output=baseline_output,
            )
        )
    current_output = baseline_output or ""
    last_result: Any = None

    for attempt in range(config.fix_max_attempts):
        log_message(
            f"🔧 Baseline fix pass {attempt + 1}/{config.fix_max_attempts} "
            f"(acceptance Gradle failed for selected SOURCE)",
            category="info",
        )
        tickets = [
            RepairTicket(
                code="baseline_preflight",
                message="Acceptance Gradle failed before plan/coder.",
                source="pipeline",
                hint=(
                    "BASELINE PREFLIGHT: repair TARGET and harness under src/test or "
                    "src/androidTest so PIPELINE VERIFY succeeds. Never edit src/main. "
                    "Then stop so Python can re-run acceptance tasks."
                ),
            ),
            *build_repair_tickets(validation_issues=[], gradle_output=current_output),
        ]
        existing = (
            file_cache.read_text(primary_target, default="")
            if Path(primary_target).is_file()
            else ""
        )
        fix_parts = build_fix_prompt(
            source_path=source_file_path,
            test_path=primary_target,
            gradle_command=gradle_command,
            kover_command=gradle_command,
            gradle_output=current_output,
            validation_issues=[],
            tickets=tickets,
            sibling_test_context=sibling_context,
            slice_tag="baseline",
            project_root=project_root,
            source_code=source_code,
            existing_test_code=existing or validation_anchor,
            target_excerpt=format_target_excerpt(existing, primary_target),
            source_categories=source_categories,
            test_layer=test_layer,
            compile_command=compile_command,
        )
        try:
            run_claude(
                fix_parts.user,
                cwd=project_root,
                title=f"BASELINE FIX PASS {attempt + 1}",
                system_prompt=fix_parts.system,
                temperature=config.fix_temperature,
                presence_penalty=config.fix_presence_penalty,
                available_tools=FIX_AVAILABLE_TOOLS,
                allowed_tools=FIX_ALLOWED_TOOLS,
                denied_tools=FIX_DENIED_TOOLS,
                timeout=agent_timeout,
                enable_thinking=config.fix_enable_thinking,
                reasoning_budget=config.fix_thinking_budget if config.fix_enable_thinking else 0,
                phase="fix",
            ...
            log_message(f"⚠️ Baseline fix pass unavailable: {exc}", category="warning")
            break

        last_result = await run_gradle_with_heartbeat(
            project_root, tasks=acceptance_tasks, offline=gradle_offline,
        )
        log_gradle_result(
            f"BASELINE GRADLE RETRY (after fix {attempt + 1})", last_result,
        )
        current_output = last_result.full_output or ""
        if is_gradle_success(last_result):
            log_message(
                f"✅ Baseline Gradle green after fix pass {attempt + 1} — "
                f"continuing {agent} plan/coder flow.",
                category="success",
            )
            return True, last_result
        if not baseline_failure_is_fixable(
            current_output,
            source_file_path=source_file_path,
            related_paths=[test_path, instr_path, primary_target],
            module_dir=owning_module,
        ):
            log_message(
                "ℹ️ Baseline retry failure no longer looks source/test related — stopping fixer.",
                category="warning",
            )
            break
...
def _load_cached_plan(source_file_path: str, test_path: str, test_state, before_gap, source_categories, sibling_context, project_root):
    ...
    try:
        ...
        target_for_demote = ""
    try:
        source_for_demote = file_cache.read_text(source_file_path)
    except file_cache.FileCacheError:
        source_for_demote = ""
    ...
    plan = ensure_not_testable_details(
        exclusive_partition_plan_lines(
            demote_partial_branch_residuals(
                plan,
                before_gap,
                target_code=target_for_demote,
                source_code=source_for_demote,
            ),
        )
    )
...
async def _run_agent_for_source_body(
    *,
    ...
    acceptance_tasks, dropped_device_tasks = filter_device_acceptance_tasks(
        acceptance_tasks, module_dir=module_dir,
    )
    ...
    if dropped_device_tasks:
        log_message(
            "📱 No existing androidTest sources in module — dropping device task(s) "
            f"(no AOSP start): {' '.join(dropped_device_tasks)}",
            category="info",
        )
    log_message(f"🧪 CLI acceptance tasks: {' '.join(acceptance_tasks) or '(none)'}", category="gradle")
    ...
    if not acceptance_tasks:
        return finish(
            ...
            failure_stage="baseline_gradle",
            evidence=(
                "No acceptance Gradle tasks left after dropping device tasks "
                "(module has no existing androidTest sources)."
            ),
        )
    ...
    if not is_gradle_success(baseline_result):
        ...
        related_paths = [test_path, instr_path, *sibling_paths]
        if (
            config.fix_max_attempts > 0
            and baseline_failure_is_fixable(
                baseline_output,
                source_file_path=source_file_path,
            ...
            log_section("BASELINE FIX (acceptance failed for selected SOURCE)", category="fix")
            repaired, retry_result = await _repair_baseline_gradle(
                project_root=project_root,
                source_file_path=source_file_path,
                source_code=source_code,
                test_path=test_path,
            ...
            if repaired and retry_result is not None:
                baseline_result = retry_result
                baseline_output = retry_result.full_output
            else:
                return finish(
                    accepted=False,
                    outcome="pipeline_unresolved",
                    failure_stage="baseline_gradle",
                    evidence=(
                        f"Baseline Gradle/Kover failed before {agent} generation "
                        "(fixer could not clear source-related acceptance errors)."
                    ),
                )
        else:
            unrelated = format_unrelated_baseline_error_message(
                baseline_output, source_file_path=source_file_path,
            )
            log_message(f"❌ {unrelated}", category="error")
            return finish(
                accepted=False,
                outcome="pipeline_unresolved",
                failure_stage="baseline_gradle",
    ...
    source_layer = None
    if run_instrumented_tests(config):
        from UnitTest_gen.core.emulator import stop_owned
        from UnitTest_gen.kotlin.layer import TestLayer, resolve_exclusive_layer
        from UnitTest_gen.kotlin.project import drop_device_acceptance_tasks

        source_layer = resolve_exclusive_layer(
            source_categories, config=config, source_file_path=source_file_path,
        ).layer
        if source_layer == TestLayer.UNIT:
            acceptance_tasks, dropped_after_baseline = drop_device_acceptance_tasks(acceptance_tasks)
            if dropped_after_baseline:
                stop_owned()
                log_message(
                    "📱 Unit-test layer selected after baseline; stopped owned emulator and "
                    f"dropped instrumented gate(s): {' '.join(dropped_after_baseline)}",
                    category="info",
                )
            before_gap_raw = unit_part or before_gap_raw
    ...
    while run_unit_tests(config) and do_coder and unit_plan.has_testable:
        ...
        suspected_source_bug = suspected_source_bug or round_out.suspected_source_bug
        if round_out.suspected_evidence:
            suspected_evidence = round_out.suspected_evidence
    ...
    if do_coder and run_instrumented_tests(config) and instr_plan.has_testable:
        ...
        if instr_round is not None:
            ...
            suspected_source_bug = suspected_source_bug or instr_round.suspected_source_bug
            if instr_round.suspected_evidence:
                suspected_evidence = instr_round.suspected_evidence

    if instr_round is not None and last_after_gap_raw is not None:
    ...
    if any_green and any_improved:
        ...
        return finish(
            accepted=True, outcome="accepted", gradle_passed=True, kover_improved=True,
            failure_stage="suspected_source_bug" if suspected_source_bug else "",
            evidence=suspected_evidence if suspected_source_bug else "",
            **common,
        )
    ...
    if suspected_source_bug:
        restore_or_delete(test_path, existed, original)
        return finish(
            accepted=False, outcome="rejected", gradle_passed=False, kover_improved=False,
            failure_stage="suspected_source_bug",
            evidence=suspected_evidence or "Suspected SOURCE contract failure (assertion trap).",
            **common,
        )
