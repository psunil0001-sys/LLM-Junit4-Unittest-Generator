...
def _android_test_runner_path(module_dir: str, project_root: str = "") -> str:
    module = Path(module_dir)
    candidates = sorted(module.glob("src/androidTest/java/**/HiltTestRunner.kt"))
    if candidates:
        return str(candidates[0].resolve())
    root = project_root or find_project_root_for_path(module_dir) or ""
    return _shared_testsupport_kt(root, "HiltTestRunner.kt")


def _shared_testsupport_kt(project_root: str, filename: str) -> str:
    """Resolve a shared harness file under ``:testsupport`` when present."""
    root = (project_root or "").strip()
    if not root or not filename:
        return ""
    base = Path(root) / "testsupport" / "src" / "main" / "java"
    if not base.is_dir():
        return ""
...
_HARNESS_PACKAGE_RE = re.compile(r"(?m)^package\s+([A-Za-z_][\w.]*)")
_FRAGMENT_EXTENDS_RE = re.compile(r":\s*Fragment\b")


@dataclass(frozen=True)
class InstrumentedHarnessNeed:
    """Source-conditioned androidTest files/deps for a Hilt Fragment CUT."""

    runner: bool = False
    manifest: bool = False
    hilt_test_activity: bool = False
    hilt_nav_test_activity: bool = False
    nav_layout: bool = False
    test_nav_graph: bool = False
    stub_home: bool = False
    plain_hilt_activity: bool = False
    uninstall_modules: tuple[str, ...] = ()
    bind_value_types: tuple[str, ...] = ()
    firebase_mocks: bool = False
    click_without_percent_visible: bool = False
    margin_layout_params: bool = False


def _harness_package_from_source(source_code: str) -> str:
    match = _HARNESS_PACKAGE_RE.search(source_code or "")
    pkg = match.group(1) if match else "com.example"
    for suffix in (
...
def _android_test_harness_dir(module_dir: str, source_code: str) -> Path:
    module = Path(module_dir)
    for name in ("HiltTestActivity.kt", "HiltTestRunner.kt", "HiltNavTestActivity.kt"):
        found = sorted(module.glob(f"src/androidTest/java/**/{name}"))
        if found:
            return found[0].parent
    pkg = _harness_package_from_source(source_code)
    return module / "src" / "androidTest" / "java" / Path(*pkg.split("."))


def required_instrumented_harness(
    source_code: str,
    *,
    categories: frozenset[str] | set[str] | None = None,
) -> InstrumentedHarnessNeed:
    """Which androidTest harness THIS SOURCE needs — not a kitchen-sink dump."""
    code = source_code or ""
    cats = set(categories or ())
    is_fragment = bool(_FRAGMENT_EXTENDS_RE.search(code)) or bool(
        cats & {"hilt_fragment", "android_fragment"}
    )
    is_hilt = "@AndroidEntryPoint" in code or "hilt_fragment" in cats
    if not (is_hilt and is_fragment):
        return InstrumentedHarnessNeed()

    needs_nav = bool(
        cats & {"android_navigation"}
        or "findNavController" in code
        or "NavDeepLinkRequest" in code
    )
    needs_home = needs_nav and (
        "NaviDestination.Home" in code or "journeylog/home" in code
    )
    needs_plain = "MainActivityDelegate" in code and (
        "IllegalArgumentException" in code or "must be MainActivityDelegate" in code
    )
    uninstall: list[str] = []
    bind_types: list[str] = []
    if "UserPreferencesRepository" in code:
        uninstall.append("PreferencesBindsModule")
        bind_types.append("UserPreferencesRepository")
    if "AuthRepository" in code:
        uninstall.append("ApiModule.BindsApiModule")
        bind_types.extend(["AuthRepository", "JourneyApi", "AuthSource"])
    firebase = any(
        token in code
        for token in (
            "Firebase.",
    ...
    dialog = "AlertDialogBuilder" in code or "AlertDialog" in code
    return InstrumentedHarnessNeed(
        runner=True,
        manifest=True,
        hilt_test_activity=True,
        hilt_nav_test_activity=needs_nav,
        nav_layout=needs_nav,
        test_nav_graph=needs_nav,
        stub_home=needs_home,
        plain_hilt_activity=needs_plain,
        uninstall_modules=tuple(uninstall),
        bind_value_types=tuple(dict.fromkeys(bind_types)),
        firebase_mocks=firebase,
        click_without_percent_visible=dialog,
        margin_layout_params=True,
    )
...
def _plain_hilt_activity_path(module_dir: str, project_root: str = "") -> str:
    module = Path(module_dir)
    candidates = sorted(module.glob("src/androidTest/java/**/PlainHiltActivity.kt"))
    if candidates:
        return str(candidates[0].resolve())
    root = project_root or find_project_root_for_path(module_dir) or ""
    return _shared_testsupport_kt(root, "PlainHiltActivity.kt")
...
def _harness_path_line(label: str, existing: str, canonical: Path, needed: bool) -> str:
    if existing:
        norm = existing.replace("\\", "/")
        if "/testsupport/" in norm:
            return f"- {label}: {existing}  (reuse :testsupport — do not Write a module copy)"
        return f"- {label}: {existing}"
    if needed:
        return f"- {label}: {canonical.resolve()}"
    return ""


def format_instrumented_bootstrap_paths_block(
    source_path: str,
    project_root: str = "",
    *,
    source_code: str = "",
    ...
    """Absolute harness paths: Read if present; Write if SOURCE requires them and they are missing."""
    root = project_root or find_project_root_for_path(source_path) or ""
    module_dir = find_owning_module_dir(root, source_path) if root else ""
    if not module_dir:
        return ""
    code = source_code or file_cache.read_text(source_path, default="")
    need = required_instrumented_harness(code, categories=categories)
    harness_dir = _android_test_harness_dir(module_dir, code)
    module = Path(module_dir)
    lines = ["INSTRUMENTED HARNESS PATHS (Read if present; Write if missing — do not guess):"]
    shared_delegate = _shared_testsupport_kt(root, "HiltDelegateActivity.kt")
    shared_carui = _shared_testsupport_kt(root, "HiltCarUiTestActivity.kt")
    for label, existing, canonical, needed in (
        (
            "HiltTestRunner",
            _android_test_runner_path(module_dir, root),
            harness_dir / "HiltTestRunner.kt",
            need.runner,
        ),
        (
            "HiltTestActivity host",
            _android_test_hilt_host_path(module_dir) or shared_delegate,
            harness_dir / "HiltTestActivity.kt",
            need.hilt_test_activity,
        ),
        (
            "HiltNavTestActivity host",
            _android_test_nav_host_path(module_dir),
            harness_dir / "HiltNavTestActivity.kt",
            need.hilt_nav_test_activity,
        ),
        (
            "androidTest manifest",
            str((module / "src" / "androidTest" / "AndroidManifest.xml").resolve())
            if (module / "src" / "androidTest" / "AndroidManifest.xml").is_file()
            else "",
        ...
        (
            "nav host layout",
            _nav_host_layout_path(module_dir),
            module / "src" / "androidTest" / "res" / "layout" / "hilt_nav_test_activity.xml",
            need.nav_layout,
        ),
        (
            "nav graph (androidTest)",
            _test_nav_graph_path(module_dir),
            module / "src" / "androidTest" / "res" / "navigation" / "hilt_nav_test_graph.xml",
            need.test_nav_graph,
        ),
        (
            "StubHomeFragment",
            _stub_home_fragment_path(module_dir),
            harness_dir / "StubHomeFragment.kt",
            need.stub_home,
        ),
        (
            "PlainHiltActivity (negative onAttach)",
            _plain_hilt_activity_path(module_dir, root),
            harness_dir / "PlainHiltActivity.kt",
            need.plain_hilt_activity,
        ),
    ):
        line = _harness_path_line(label, existing, canonical, needed)
        if line:
            lines.append(line)
    if shared_delegate and need.hilt_test_activity:
        lines.append(
            f"- HiltDelegateActivity (Tier A attach): {shared_delegate}  "
            "(reuse :testsupport)"
        )
    if shared_carui:
        lines.append(
            f"- HiltCarUiTestActivity: {shared_carui}  "
            "(reuse :testsupport — do not Write a module copy)"
        )
    nav_graph = _nav_graph_path(module_dir, source_path)
    if nav_graph:
        lines.append(f"- nav graph (main): {nav_graph}")
    ...
    if need.runner:
        lines.append(
            "- gradle: testInstrumentationRunner = "
            "(or module runner if listed above); "
            "espresso-core:3.7.0; androidx.test:runner:1.6.2; "
            "androidx.test.ext:junit:1.2.1; hilt-android-testing:2.49; mockk-android"
        )
    if need.uninstall_modules:
        lines.append(
            "- UninstallModules: "
            + ", ".join(need.uninstall_modules)
            + " — BindValue: "
            + ", ".join(need.bind_value_types)
        )
    if need.firebase_mocks:
        lines.append(
            "- Firebase: mockkStatic(FirebaseApp/Crashlytics/Analytics) in @Before; "
            "do not FirebaseApp.initializeApp"
        )
    if need.click_without_percent_visible:
        lines.append(
            "- AlertDialog Espresso: clickWithoutPercentVisible() ViewAction "
            "(dialog buttons fail 90% visibility on car displays)"
        )
    if need.margin_layout_params:
        lines.append(
            "- HiltTestActivity FragmentContainerView must have MarginLayoutParams "
...
def _test_tree_for_output(output_file_path: str | None) -> str:
    path = (output_file_path or "").replace("\\", "/")
    return "androidTest" if "/androidTest/" in path else "test"

def _hilt_test_activity_candidates(module_dir: str, *, test_tree: str = "test") -> list[Path]:
    test_src_root = os.path.join(module_dir, "src", test_tree, "java")
...
def module_has_manifest_declared_hilt_test_activity(
    ...
    if not module_dir:
        return False
    test_tree = _test_tree_for_output(output_file_path)
    manifest_path = os.path.join(module_dir, "src", test_tree, "AndroidManifest.xml")
    ...
    if "HiltTestActivity" not in manifest_content:
        return False
    for candidate in _hilt_test_activity_candidates(module_dir, test_tree=test_tree):
...
def format_sibling_test_paths(sibling_paths: list[str]) -> str:
    ...
    if not sibling_paths:
        return (
            "SIBLING TEST FILES: none listed. Do not Glob/Bash the repo looking for *Test.kt. "
            "Invent harness from SOURCE, recipes, generated sources, and docs already in this prompt.\n"
        )
    lines = [
        "SIBLING TEST FILES (absolute paths — optional host-setup reference ONLY, NEVER edit)",
        "If listed: Read at most one sibling for host/runner/@Config patterns; do not copy whole suites.",
...
def module_has_android_tests(module_dir: str | Path | None) -> bool:
    """True when ``src/androidTest`` has at least one real test (``*Test.kt`` or ``@Test``)."""
    if not module_dir:
        return False
    root = Path(module_dir) / "src" / "androidTest"
    if not root.is_dir():
        return False
...
def filter_device_acceptance_tasks(
    tasks: list[str] | None,
    *,
    module_dir: str | Path | None,
) -> tuple[list[str], list[str]]:
    """Drop connected/instrumentedCoverage tasks when the module has no androidTests.

    Returns ``(kept_tasks, dropped_tasks)``. Non-device tasks are always kept.
    """
    from UnitTest_gen.core.emulator import gradle_needs_device

    ordered = [str(task).strip() for task in (tasks or []) if str(task).strip()]
    if not ordered:
        return [], []
    if module_has_android_tests(module_dir):
        return ordered, []
    kept: list[str] = []
    dropped: list[str] = []
    for task in ordered:
        if gradle_needs_device([task]):
            dropped.append(task)
        else:
            kept.append(task)
    return kept, dropped
...
def drop_device_acceptance_tasks(tasks: list[str] | None) -> tuple[list[str], list[str]]:
    """Remove device-only Gradle tasks after a source is assigned to unit tests."""
    from UnitTest_gen.core.emulator import gradle_needs_device
