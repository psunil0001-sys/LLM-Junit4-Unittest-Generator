...
def format_api_doc_catalog(
    ...
    *,
    ...
    if TestLayer.normalize(test_layer) == TestLayer.INSTRUMENTED:
        ...
        header = (
            "VERIFIED INSTRUMENTED LIBRARY API DOCS — optional androidTest cheat-sheets; "
            "prefer SOURCE + generated sources + in-repo siblings when they disagree.",
            "view or grep ONE listed file when you need a signature "
            "(prefer grep for a class/method name; view only if needed). "
            "Recipes are ordered-write patterns; api_docs are optional signatures.",
        )
    else:
        paths = select_api_doc_files(source_code, categories=categories)
        header = (
            "VERIFIED LIBRARY API DOCS — optional cheat-sheets. Prefer in-repo siblings + "
            "generated sources + SOURCE when they disagree.",
            "view or grep ONE listed file when you need a signature "
            "(prefer grep for a class/method name; view only if needed). "
            "Recipes are ordered-write patterns; api_docs are optional signatures.",
        )
...
_COMPANION_RECIPES = frozenset({
    ...
    "osmdroid_mapview.md",
    "flow_builder_cancellation_branch.md",
})
...
_PRIMARY_TO_LANE: dict[str, str] = {
    ...
    "complex_viewmodel_android.md": "instrumented_viewmodel",
...
def _select_primary_recipe(
    ...
    *,
    ...
    if "complex_android_viewmodel" in tags:
        return _pick("complex_viewmodel_android.md", "instrumented_viewmodel")
    if "complex_android_fragment" in tags:
        return _pick("hilt_fragment_activity_viewmodels.md", "hilt_fragment")
...
def _is_thin_flow_emit_wrapper(source_code: str) -> bool:
    """True when SOURCE builds Flows with ``flow { emit(...) }`` (repo-style wrappers)."""
    hay = source_code or ""
...
def _select_companion_recipes(
    ...
    *,
    ...
    hay_l = hay.lower()
    candidates: list[str] = []
    # Prefer flow-cancel companion early so plain_jvm Flow wrappers always get a slot.
    if _is_thin_flow_emit_wrapper(hay) or (
        "coroutines_flow" in tags and "flow {" in hay and "emit(" in hay
    ):
        candidates.append("flow_builder_cancellation_branch.md")
...
def _keep_heading(title: str, *, stubs_only: bool = False) -> bool:
    ...
    if stubs_only:
        return False
    return (
        "ordered write steps" in lower
        or "full coverage checklist" in lower
        or "module harness" in lower
    )
...
def format_recipe_catalog(
    ...
    *,
    ...
    lines = [
        "VERIFIED TEST RECIPES — ordered-write patterns. Adapt names to THIS SOURCE. "
...
def format_instrumented_recipe_catalog(
    ...
    *,
    ...
    lines = [
        "VERIFIED INSTRUMENTED TEST RECIPES — androidTest ordered-write patterns; "
        "do not use Robolectric or src/test. Adapt names to THIS SOURCE.",
...
def format_fix_recipe_context(
    ...
    *,
    ...
    lines = [
        "FIX RECIPE CONTEXT — re-read harness/recipe paths below; follow SOURCE/generated "
        "evidence over a recipe step if they disagree.",
