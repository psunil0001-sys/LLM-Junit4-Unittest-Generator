...
INSTRUMENTED_CATEGORIES = frozenset({
    ...
    "bluetooth_hardware",
    "camera_hardware",
    "complex_android_viewmodel",
    "complex_android_fragment",
})
...
def resolve_exclusive_layer(
    ...
    *,
    ...
    tags = frozenset(categories or ())
    if "viewmodel" in tags and tags & {"hilt_viewmodel", "hilt_entrypoint", "android_context", "livedata", "delegated_viewmodel_drive"}:
        tags = tags | {"complex_android_viewmodel"}
    if "android_fragment" in tags and tags & {"hilt_entrypoint", "android_navigation", "delegated_viewmodel_fragment", "carui_toolbar"}:
        tags = tags | {"complex_android_fragment"}
    ...
    if tags & INSTRUMENTED_CATEGORIES:
        return LayerDecision(
            ...
            f"hard rule: instrumented categories {sorted(tags & INSTRUMENTED_CATEGORIES)}; friction override",
        )
...
def _item_layer(
    ...
    *,
    ...
    if frozenset(categories or ()) & INSTRUMENTED_CATEGORIES:
        return TestLayer.INSTRUMENTED
    if default == TestLayer.INSTRUMENTED:
        return TestLayer.INSTRUMENTED
