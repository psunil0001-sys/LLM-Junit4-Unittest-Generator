# Proprietary — Sunilkumar Pathipati. Thin CLI coordinator for ADK Kotlin test generation.
"""Public CLI and per-target loop for the ADK agent pipeline."""
...
async def async_main():
    parser = argparse.ArgumentParser(
        description=(
            "AI Android Test Generator powered by Google ADK agents + OpenAI-compatible LLM "
            "(llama.cpp or remote vLLM). The agent writes tests; Python verifies Gradle/Kover."
        )
    )
    parser.add_argument(
        "--env",
        ...
        choices=("adk", "claude"),
        default=None,
        help="Agent runtime (ADK). Shared llama :8080/v1. "
    ...
    parser.add_argument(
        ...
        choices=("llama", "vllm"),
        default=None,
        help="LLM upstream: local llama.cpp (default) or remote OpenAI-compatible vLLM.",
    )
    parser.add_argument(
        "--vllm-base-url",
        type=str,
        default=None,
        help="Remote vLLM/Cloudflare root URL (with or without /v1). Requires --llm-backend vllm.",
    )
    parser.add_argument(
        "--vllm-api-key",
        type=str,
        default=None,
        help="Bearer API key for the remote vLLM endpoint (TESTGEN_VLLM_API_KEY).",
    )
    parser.add_argument(
    ...
    stream_group.add_argument(
        ...
        action="store_true",
        default=None,
        help="Stream model reasoning/content tokens and tool lines live in the terminal "
        "(default on; also TESTGEN_AGENT_STREAM=1).",
    )
    stream_group.add_argument(
        "--no-stream",
        action="store_true",
        help="Disable token streaming; wait for each full LLM turn (old heartbeat mode).",
    )
    parser.add_argument(
        "--stop-server-on-exit",
        action="store_true",
        help="Stop the local llama-server when this process exits, even if it was "
        "already running before this script started. A server started by this "
        "script is always stopped on exit / Ctrl+C.",
    )
    ...
    def _handle_interrupt(signum, frame):
        # Stop owned llama-server and AOSP emulator immediately so they do not
        # keep running after Ctrl+C / SIGTERM while the async pipeline unwinds.
        from UnitTest_gen.core.emulator import stop_owned
        from UnitTest_gen.core.llm import stop_owned_coding_server
    ...
    from UnitTest_gen.core.config import normalize_llm_backend

    started_server = False
    try:
        if normalize_llm_backend(pipeline_config.llm_backend) == "vllm":
            if not pipeline_config.vllm_base_url.strip():
                print("❌ --llm-backend vllm requires --vllm-base-url / TESTGEN_VLLM_BASE_URL.")
                return False
            try:
                ...
                print(f"❌ {exc}")
                return False
            if not args.skip_server_preflight and not is_server_healthy():
                print(
                    "❌ Remote vLLM endpoint is not healthy.\n"
                    f"   Check TESTGEN_VLLM_BASE_URL ({pipeline_config.vllm_base_url}) "
                    "and TESTGEN_VLLM_API_KEY."
                )
                return False
        elif pipeline_config.auto_start_local_llm and not is_server_healthy():
            try:
                resolve_agent_model(pipeline_config)
            except RuntimeError as exc:
                print(f"❌ {exc}")
        ...
        # Always stop a server this process started; --stop-server-on-exit also
        # stops a pre-existing local server the user asked to tear down.
        # Emulator started for -G device tasks stays up until this exit.
        from UnitTest_gen.core.emulator import stop_owned
        from UnitTest_gen.core.llm import coding_server_owned, stop_coding_server, stop_owned_coding_server

        stop_owned()
        if started_server or coding_server_owned():
            stop_owned_coding_server()
        elif args.stop_server_on_exit:
            stop_coding_server()
