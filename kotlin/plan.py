...
def format_plan_slice_for_coder(
    ...
    # The re-embedded Markdown body is dropped: its per-TC lines/scenario/given/when/then
    # duplicate the ORDERED APPROACH items (per-item lines + approach + fixture + risks) and
    # the slice line-list below, tripling the DELTA line-list. Keep the authoritative pointer
    # + the unique ORDERED APPROACH / LATER SLICES / ALREADY DEFERRED blocks.
    _ = md_text
    ...
    header = [
        ...
        "Implement only the ORDERED APPROACH items whose lines fall in this slice.",
    ...
    if partial:
        ...
        items.append(
            PlanGapItem(
                ...
                reason=(
                    f"{base}. Partial-branch probes (Kover mb>0) were never classified; "
                    "treat as residual until planner emits Markdown with per-branch reasons "
    ...
    plan = ensure_not_testable_details(
        exclusive_partition_plan_lines(
            demote_partial_branch_residuals(
                plan, gap, target_code=existing_for_lane, source_code=source_code,
            ),
        )
    )
    from UnitTest_gen.core.config import get_config
...
def _source_is_thin_flow_emit_wrapper(source_code: str) -> bool:
    """True when SOURCE builds Flows with ``flow { emit(...) }``."""
    hay = source_code or ""
    return bool(re.search(r"flow\s*\{[^}]*\bemit\s*\(", hay, re.DOTALL))

def _target_has_flow_cancel_probe(target_code: str) -> bool:
    """True when TARGET already uses launchIn/cancelAndJoin cancel probes."""
    hay = target_code or ""
    return "launchIn" in hay and ("cancelAndJoin" in hay or "cancel()" in hay)
...
def _residual_reason(
    *,
    ...
    methods: tuple[str, ...],
    test_names: list[str],
    flow_cancel_attempted: bool = False,
) -> str:
    ...
    if flow_cancel_attempted:
        return (
            f"Kover residual branch (mb>0) on lines {line_txt}: TARGET already exercises the "
            f"public path via {tests}{extra} and includes flow cancel probes "
            f"(launchIn/cancelAndJoin) and/or exception arms. Methods: {method_txt}. "
            "Remaining probe is a synthetic/residual Kover branch after cancel/exception "
            "attempts; no additional complementary-arm unit test planned."
        )
    return (
        f"Kover residual branch (mb>0) on lines {line_txt}: TARGET already exercises the "
...
def demote_partial_branch_residuals(
    ...
    *,
    ...
    For thin ``flow { emit(...) }`` wrappers, do **not** demote until TARGET already
    includes cancel/slow-collector probes (``launchIn`` / ``cancelAndJoin``) — those
    close most coroutine state-machine branches that ``.first()`` alone leaves open.
    """
    ...
    flow_emit = _source_is_thin_flow_emit_wrapper(source)
    flow_cancel_done = _target_has_flow_cancel_probe(target)
    kept: list[PlanGapItem] = []
    ...
    for item in plan.testable:
        ...
        if item.lines and not (item_line_set - partial):
            if flow_emit and not flow_cancel_done:
                # Keep testable so planner/coder apply flow_builder_cancellation_branch.
                kept.append(item)
                continue
            ...
            demote_reason = _residual_reason(
                ...
                test_names=tests,
                flow_cancel_attempted=flow_emit and flow_cancel_done,
            )
...
def build_plan_prompt(
    *,
    ...
    dynamic_parts.extend([
        # One merged layer block (guidance + policy); "Python assigns test_layer after
        # clamp" and "one layer per item" are stated once here, not per source block.
        f"TEST LAYER (exclusive — one layer per plan item; do not set test_layer in frontmatter)",
        f"- RESOLVED PRIMARY LAYER: {layer_decision.layer.value} "
        f"(unit_friction={layer_decision.unit_friction}, "
        f"instrumented_friction={layer_decision.instrumented_friction}; reason: {layer_decision.reason})",
        ...
        "- Use ONLY the harness and recipes for the assigned test_layer; plan scenarios only.",
        "- Python assigns test_layer per item after clamp; never plan the same delta lines "
        "for both layers.",
    ])
    return PromptParts(
