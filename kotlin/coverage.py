...
class KoverGap:
    ...
    metric_elements: dict[str, tuple[set[str], set[str]]] = field(default_factory=dict)
...
def is_generated_kover_source(package_name: str, source_file_name: str) -> bool:
    ...
    if package_path.startswith("dagger/hilt/internal") or package_path.startswith("hilt_aggregated_deps"):
        return True
    if "build/generated" in package_path or package_path.endswith("/databinding"):
        return True
    if source_name in {"BuildConfig", "R", "Manifest"} or source_name.startswith(("R$", "_")):
        return True
    if source_file_name.startswith("Hilt_"):
        return True
    if source_name.endswith(("Binding", "BindingImpl", "_MembersInjector", "_GeneratedInjector", "_Factory")):
        return True
    return False
...
def _parse_sourcefile_line_gaps(
    source_node,
) -> tuple[set[int], set[int], dict[int, LineGapDetail], set[int]]:
    missed_lines, missed_branches, line_details, covered_lines = set(), set(), {}, set()
    ...
    return missed_lines, missed_branches, line_details, covered_lines
...
def _sourcefile_metric_elements(package_node, source_file_name: str) -> dict[str, tuple[set[str], set[str]]]:
    """Return stable covered/total element keys for cross-report unions.

    JaCoCo XML does not expose instruction or branch ids. Its per-method and
    per-line counters are the finest identities available, so deterministic
    slots preserve overlap without pretending that aggregate counts are
    independent code elements.
    """
    elements = {metric: (set(), set()) for metric in ("class", "method", "branch", "instruction")}
    for class_node in package_node.findall("class"):
        if class_node.attrib.get("sourcefilename") != source_file_name:
            continue
        class_name = class_node.attrib.get("name", "")
        class_missed, class_covered = counter_pair(class_node, "INSTRUCTION")
        class_key = f"{package_node.attrib.get('name', '')}/{class_name}"
        class_is_covered = class_covered > 0
        elements["class"][1].add(class_key)
        if class_is_covered:
            elements["class"][0].add(class_key)
        for method in class_node.findall("method"):
            method_key = f"{class_key}::{method.attrib.get('name', '')}{method.attrib.get('desc', '')}"
            method_missed, method_covered = counter_pair(method, "LINE")
            if method_missed + method_covered == 0:
                method_missed, method_covered = counter_pair(method, "INSTRUCTION")
            elements["method"][1].add(method_key)
            if method_covered > 0:
                elements["method"][0].add(method_key)
            for line in method.findall("line"):
                line_no = line.attrib.get("nr", "0")
                covered_branches = int(line.attrib.get("cb", "0") or "0")
                missed_branches = int(line.attrib.get("mb", "0") or "0")
                for index in range(covered_branches + missed_branches):
                    key = f"{method_key}@{line_no}:branch:{index}"
                    elements["branch"][1].add(key)
                    if index < covered_branches:
                        elements["branch"][0].add(key)
                instruction_missed, instruction_covered = counter_pair(line, "INSTRUCTION")
                for index in range(instruction_covered + instruction_missed):
                    key = f"{method_key}@{line_no}:instruction:{index}"
                    elements["instruction"][1].add(key)
                    if index < instruction_covered:
                        elements["instruction"][0].add(key)
        # Some reports only expose counters on the class/sourcefile node.
        for metric in ("instruction",):
            counter_missed, counter_covered = counter_pair(class_node, metric.upper())
            for index in range(counter_covered + counter_missed):
                key = f"{class_key}:{metric}:{index}"
                elements[metric][1].add(key)
                if index < counter_covered:
                    elements[metric][0].add(key)
    source_node = package_node.find(f"sourcefile[@name='{source_file_name}']")
    if source_node is not None:
        source_key = f"{package_node.attrib.get('name', '')}/{source_file_name}"
        for line in source_node.findall("line"):
            line_no = line.attrib.get("nr", "0")
            covered_branches = int(line.attrib.get("cb", "0") or "0")
            missed_branches = int(line.attrib.get("mb", "0") or "0")
            for index in range(covered_branches + missed_branches):
                key = f"{source_key}@{line_no}:branch:{index}"
                elements["branch"][1].add(key)
                if index < covered_branches:
                    elements["branch"][0].add(key)
        if not elements["instruction"][1]:
            missed, covered = counter_pair(source_node, "INSTRUCTION")
            for index in range(covered + missed):
                key = f"{source_key}:instruction:{index}"
                elements["instruction"][1].add(key)
                if index < covered:
                    elements["instruction"][0].add(key)
    return elements
    ...
    for package in xml_root.findall("package"):
        ...
        for sourcefile in package.findall("sourcefile"):
            ...
            if not include_generated and is_generated_kover_source(package_name, source_file_name):
                continue
            missed_lines, missed_branches, _, covered_lines = _parse_sourcefile_line_gaps(sourcefile)
            ...
            metric_elements = _sourcefile_metric_elements(package, source_file_name)
            ...
            gaps[key] = KoverGap(
                ...
                line_missed=line_missed, branch_missed=branch_missed,
                covered_lines=covered_lines,
                metric_elements=metric_elements,
            )
    return gaps
...
def parse_kover_gap(xml_path: str, source_file_path: str, source_code: str) -> CoverageGap | None:
    ...
    if source_node is None or package_node is None:
        return None
    missed_line_set, partial_branch_set, line_details, _ = _parse_sourcefile_line_gaps(source_node)
...
def entry_lines_and_branches(entry: dict[str, Any] | None) -> tuple[set[int], set[int]]:
    """Split an unreachable row into missed-line vs partial-branch probe sets.

    Legacy rows with ``category=kover_residual_branch`` and only ``lines`` populated
    are treated as branch probes (those numbers were partial-branch line nos).
    """
    if not isinstance(entry, dict):
        return set(), set()
    lines = coerce_int_set(entry.get("lines"))
    branches = coerce_int_set(entry.get("branches"))
    category = str(entry.get("category") or "").strip()
    if not branches and category == "kover_residual_branch" and lines:
        return set(), lines
    return lines, branches


def deferred_lines_for_source(source_file_path: str, *, path: Path | str | None = None) -> set[int]:
    """Deferred *missed-line* probes for ``source`` (legacy name kept for callers)."""
    return deferred_probes_for_source(source_file_path, path=path)[0]


def deferred_probes_for_source(
    source_file_path: str, *, path: Path | str | None = None
) -> tuple[set[int], set[int]]:
    """Return ``(deferred_missed_lines, deferred_partial_branch_lines)`` for ``source``."""
...
def filter_gap(gap, source_file_path: str, *, path: Path | str | None = None):
    """Return a gap with deferred missed-lines / partial-branches removed."""
    if gap is None:
        return None
    deferred_lines, deferred_branches = deferred_probes_for_source(source_file_path, path=path)
    if not deferred_lines and not deferred_branches:
        return gap
    missed = [n for n in (getattr(gap, "missed_lines", None) or []) if int(n) not in deferred_lines]
    partial = [
        n
        for n in (getattr(gap, "partial_branch_lines", None) or [])
        if int(n) not in deferred_branches
    ]
...
def _entry_probe_key(
    source: str, lines, branches
) -> tuple[str, tuple[int, ...], tuple[int, ...]]:
    return (
        os.path.abspath(source or ""),
        tuple(sorted(coerce_int_set(lines))),
        tuple(sorted(coerce_int_set(branches))),
    )

def merge_unreachable_entries(entries: list[dict[str, Any]], *, path: Path | str | None = None) -> Path:
    """Merge unreachable rows by source + sorted lines + sorted branches."""
    store = load_store(path)
    by_key: dict[tuple[str, tuple[int, ...], tuple[int, ...]], dict[str, Any]] = {}
    ordered: list[dict[str, Any]] = []
    for row in store.get("entries") or []:
        if not isinstance(row, dict):
            continue
        row_lines, row_branches = entry_lines_and_branches(row)
        normalized = dict(row)
        normalized["lines"] = sorted(row_lines)
        normalized["branches"] = sorted(row_branches)
        key = _entry_probe_key(str(normalized.get("source") or ""), row_lines, row_branches)
        if not key[0] or (not key[1] and not key[2]) or key in by_key:
            continue
    ...
    for entry in entries:
        ...
        key = _entry_probe_key(source, lines_set, branches_set)
        if not source or (not lines_set and not branches_set):
            continue
...
def reconcile_unreachable_entries(
    *,
    ...
    attempted_branches: set[int] | list[int] | tuple[int, ...] = (),
    covered_branches: set[int] | list[int] | tuple[int, ...] = (),
    accepted: bool = False,
    ...
    evidence_branches = coerce_int_set(attempted_branches) | coerce_int_set(covered_branches)
    if not evidence_lines and not evidence_branches:
        return None
    store = load_store(path)
    ...
    for row in store.get("entries") or []:
        ...
        should_drop = (
            ...
            and (bool(row_lines & evidence_lines) or bool(row_branches & evidence_branches))
            and (
                accepted
                or bool(coerce_int_set(covered_lines))
                or bool(coerce_int_set(covered_branches))
            )
        )
        if should_drop:
            changed = True
...
def _classify_plan_item_probes(item: dict[str, Any]) -> tuple[list[int], list[int]]:
    """Return ``(missed_lines, partial_branch_lines)`` for a not_testable plan item dict."""
    if not isinstance(item, dict):
        return [], []
    has_branches_key = "branches" in item
    has_lines_key = "lines" in item
    if has_branches_key and has_lines_key:
        return sorted(coerce_int_set(item.get("lines"))), sorted(coerce_int_set(item.get("branches")))
    if has_branches_key and not has_lines_key:
        return [], sorted(coerce_int_set(item.get("branches")))
    nums = sorted(coerce_int_set(item.get("lines")))
    category = str(item.get("category") or "").strip()
    reason = str(item.get("reason") or "").lower()
    if category == "kover_residual_branch" or "partial-branch" in reason or "partial branch" in reason:
        return [], nums
    return nums, []
...
def entries_from_plan_not_testable(
    ...
    *,
    ...
    for item in not_testable or []:
        if not isinstance(item, dict):
            continue
        lines, branches = _classify_plan_item_probes(item)
        category = str(item.get("category") or "not_unit_testable").strip() or "not_unit_testable"
        reason = str(item.get("reason") or "").strip()
        if not reason:
            probe_bits = []
            if lines:
                probe_bits.append(f"lines {lines}")
            if branches:
                probe_bits.append(f"branches {branches}")
            reason = (
                f"Deferred ({category}): {', '.join(probe_bits) or 'none'} "
                "marked not_testable by planner."
            )
        rows.append(
            {
                "source": source,
...
def entries_from_attempted_no_delta(
    ...
    *,
    ...
    """Legacy helper: planned probes still open after Python Coverage no-delta."""
    if after_gap is None or not testable:
        return []
    still_missed = coerce_int_set(getattr(after_gap, "missed_lines", None))
    still_partial = coerce_int_set(getattr(after_gap, "partial_branch_lines", None))
    still_open = still_missed | still_partial
    if not still_open:
        return []
    source = os.path.abspath(source_file_path)
    ...
    for item in testable:
        ...
        open_nums = planned & still_open
        if not open_nums:
            continue
        open_lines = sorted(open_nums & still_missed)
        open_branches = sorted(open_nums & still_partial)
        ...
        if open_lines:
            reason_parts.append(f"Still-open missed lines after attempt: {open_lines}.")
        if open_branches:
            reason_parts.append(f"Still-open partial-branch probes after attempt: {open_branches}.")
        rows.append(
            {
                "source": source,
                "source_name": os.path.basename(source),
...
SUSPECTED_BUGS_JSON = PACKAGE_DIR / "data" / "suspected_bugs.json"
SUSPECTED_BUGS_DIR = PACKAGE_DIR / "data" / "suspected_bugs"
_SUSPECTED_SCHEMA = "1.0"


def suspected_bugs_store_path() -> Path:
    override = os.environ.get("TESTGEN_SUSPECTED_BUGS_JSON", "").strip()
    return Path(override).expanduser().resolve() if override else SUSPECTED_BUGS_JSON


def suspected_bugs_dir() -> Path:
    override = os.environ.get("TESTGEN_SUSPECTED_BUGS_DIR", "").strip()
    return Path(override).expanduser().resolve() if override else SUSPECTED_BUGS_DIR


def _empty_suspected_bugs() -> dict[str, Any]:
    return {"schema_version": _SUSPECTED_SCHEMA, "updated_at": "", "entries": []}


def load_suspected_bugs(path: Path | str | None = None) -> dict[str, Any]:
    target = Path(path) if path else suspected_bugs_store_path()
    payload = file_cache.read_json(target, default_factory=_empty_suspected_bugs)
    if not isinstance(payload, dict) or not isinstance(payload.get("entries"), list):
        return _empty_suspected_bugs()
    payload.setdefault("schema_version", _SUSPECTED_SCHEMA)
    return payload


def _bug_key(source: str, test_file: str) -> tuple[str, str]:
    return (os.path.abspath(source or ""), os.path.abspath(test_file or ""))


def persist_suspected_bug(
    *,
    source_file_path: str,
    test_file_path: str,
    test_code: str,
    module: str = "",
    slice_tag: str = "",
    lines: list[int] | set[int] | tuple[int, ...] = (),
    gradle_output: str = "",
    store_path: Path | str | None = None,
    sidecar_dir: Path | str | None = None,
) -> dict[str, Any]:
    """Copy TARGET to a sidecar and upsert suspected_bugs.json. Returns the ledger row."""
    source = os.path.abspath(source_file_path)
    test_file = os.path.abspath(test_file_path)
    dest_dir = Path(sidecar_dir) if sidecar_dir else suspected_bugs_dir()
    dest_dir.mkdir(parents=True, exist_ok=True)
    sidecar = dest_dir / f"{Path(source).stem}__{Path(test_file).stem}.kt"
    sidecar.write_text(test_code or "", encoding="utf-8")
    file_cache.invalidate(str(sidecar))
    snippet = (gradle_output or "").strip()
    if len(snippet) > 800:
        snippet = snippet[:400] + "\n...\n" + snippet[-350:]
    line_list = sorted(coerce_int_set(lines))
    recorded = datetime.now().isoformat(timespec="seconds")
    row = {
        "source": source,
        "source_name": os.path.basename(source),
        ...
        "slice_tag": slice_tag,
        "lines": line_list,
        "gradle_snippet": snippet,
        "updated_at": recorded,
    }
    store = load_suspected_bugs(store_path)
    key = _bug_key(source, test_file)
    entries: list[dict[str, Any]] = []
    replaced = False
    for existing in store.get("entries") or []:
        if not isinstance(existing, dict):
            continue
        if _bug_key(str(existing.get("source") or ""), str(existing.get("test_file") or "")) == key:
            existing.update(row)
    ...
    save_json(Path(store_path) if store_path else suspected_bugs_store_path(), store, touch_updated_at=True)
    return row
